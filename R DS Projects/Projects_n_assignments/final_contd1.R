...
tweet_file_sample <- as.data.frame(tweet_file[rbinom(nrow(tweet_file)*.5,nrow(tweet_file),.5), ])

#blog_file <- read_file("en_US.blogs.txt")
blog_file_sample <- as.data.frame(blog_file[rbinom(nrow(blog_file)*.5,nrow(blog_file),.5), ])

#news_file <- read_file("en_US.news.txt")
news_file_sample <- as.data.frame(news_file[rbinom(nrow(news_file)*.5,nrow(news_file),.5), ])

colnames(tweet_file_sample) <- c("text")
colnames(news_file_sample) <- c("text")
colnames(blog_file_sample) <- c("text")
...
# first pass blog file sample alone
twitter_file <- blog_file_sample
